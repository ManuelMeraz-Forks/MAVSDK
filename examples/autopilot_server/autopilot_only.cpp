//
// Created by paul.weidinger on 5/10/22.
//
...
{
    std::string port = "14551";
    if (argc > 1) {
        port = std::string(argv[argc - 1]);
    }

    Mavsdk mavsdkTester;
    const Mavsdk::Configuration configuration(Mavsdk::Configuration::UsageType::Autopilot);
    mavsdkTester.set_configuration(configuration);

    std::stringstream connection_url;
    connection_url << "udp://127.0.0.1:";
    connection_url << port;
    const auto result = mavsdkTester.add_any_connection(connection_url.str());
    ...
    mavsdkTester.subscribe_on_new_system([&mavsdkTester, &prom]() {
        std::cout << "Discovered MAVSDK FCS/GCS" << std::endl;
    ...
    auto paramServer = ParamServer{system};
    auto telemServer = TelemetryServer{system};
    auto actionServer = ActionServer{system};
    ...
    missionRawServer.subscribe_incoming_mission(
        [&mission_prom,
         &missionRawServer](MissionRawServer::Result res, const MissionRawServer::MissionPlan& plan) {
    ...
    TelemetryServer::RawGps rawGps{0, 55.953251, -3.188267, 0, NAN, NAN, 0, NAN, 0, 0, 0, 0, 0, 0};
